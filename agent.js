/**
 * CyberAgent - Advanced AI Assistant
...
class CyberAgent {
  constructor() {
    ...
    this.debouncedUpdateModelOptions = this.debounce(
  ...
  async init() {
    try {
      ...
      console.log(`CyberAgent v${this.version} initialized successfully`);
    } catch (error) {
      console.error("Failed to initialize CyberAgent:", error);
      this.showToast(
        "error",
        "Initialization Error",
        `Failed to start the application: ${error.message || error}`
      );
      try {
        this.hideLoadingScreen();
      } catch (_) {}
  ...
  async showLoadingScreen() {
    ...
    const progressBar = loadingScreen
      ? loadingScreen.querySelector(".loading-progress")
      : null;
  ...
  initializeThemeDetection() {
    try {
      // Always set dark theme for cyberpunk mode
    ...
    try {
      ...
      if (performance && performance.memory) {
        this.memoryMonitor = setInterval(() => {
          try {
            this.state.performance.memoryUsage = Math.round(
  ...
  setupEventListeners() {
    try {
      ...
      document
        ...
        ?.addEventListener("click", () => this.clearConversationMessages());
      document
        .getElementById("toggle-perf")
        ?.addEventListener("click", () => this.togglePerformanceMonitor());
      document
        .getElementById("close-settings")
        ?.addEventListener("click", () => this.closeSettings());
      document
        .getElementById("save-settings")
        ?.addEventListener("click", () => this.saveAndApplySettings());
      document
        .querySelector(".toggle-visibility")
        ?.addEventListener("click", (e) => this.toggleApiKeyVisibility(e));
      document
        .getElementById("llm-provider")
        ?.addEventListener("change", () => this.updateModelOptions());
      document
        .getElementById("api-key")
        ?.addEventListener("input", this.debouncedUpdateModelOptions);
      // Temperature range value update
      const tempInput = document.getElementById("temperature");
      if (tempInput) {
        tempInput.addEventListener("input", () => {
          const valueSpan = tempInput.parentNode.querySelector(".range-value");
          if (valueSpan) valueSpan.textContent = tempInput.value;
        });
      }
  ...
  async sendMessage() {
    ...
    this.updateUIState();
    let convId =
      this.state.currentConversationId || this.createNewConversation();
    ...
    while (maxTurns-- > 0) {
      ...
      try {
        ...
        this.state.performance.apiCalls =
          (this.state.performance.apiCalls || 0) + 1;
        this.updatePerformanceDisplay();

        const response = this.parseAPIResponse(
          responseData,
          this.state.settings.llm.provider
        );
        ...
        if (response && response.tool_calls && response.tool_calls.length > 0) {
          this.addMessage(
            "assistant",
            `Using tools: ${response.tool_calls
        ...
        this.addMessage(
          "system",
          `Agent iteration error: ${err.message || err}`,
          conversationId
        );
        break;
      }
  ...
  async callLLM(conversation) {
    ...
    const { provider, apiKey, model, maxTokens, temperature } =
        ...
        choices: [
          {
            message: {
              content:
                "Demo response: provide an API key in settings to use real models.",
            },
          },
        ],
      };
    ...
    switch (provider) {
      ...
      case "google":
        ...
        const systemMessages = messagesForApi
          .filter((m) => m.role === "system")
          .map((m) => m.content)
          .join("\n");

        // Convert non-system messages to Google format
        const contents = messagesForApi
          .filter((m) => m.role !== "system") // Google handles system messages separately
          .map((m) => ({
  ...
  displayMessage(message) {
    try {
      ...
      const senderName =
        { user: "You", assistant: "CyberAgent", system: "System", tool: "Tool" }[
          message.role
        ] || message.role;
      const avatarIcon =
        {
      ...
      if (typeof message.content === "string" && window.marked) {
        ...
        processedContent = `<pre><code>${this.escapeHtml(
          JSON.stringify(message.content, null, 2)
        )}</code></pre>`;
      }

      messageEl.innerHTML = `<div class="message-header"><div class="message-avatar ${
        message.role
      }"><i class="fas ${avatarIcon}"></i></div><div class="message-info"><div class="message-sender">${this.escapeHtml(
        senderName
      )}</div></div></div><div class="message-content">${processedContent}</div>`;
  ...
  async loadConversationHistory() {
    try {
      ...
      if (stored) {
        ...
        const recent = Array.from(this.state.conversations.values()).sort(
    ...
    try {
      if (this.state.settings.advanced.autoSave) {
        ...
        localStorage.setItem(
          "agentflow_conversations",
          JSON.stringify(Array.from(this.state.conversations.entries()))
        );
      }
    } catch (e) {
  ...
  populateSettingsForm() {
    ...
    if (llmProv) llmProv.value = s.llm.provider || "google";
  ...
  updateSettingsFromForm() {
    const s = this.state.settings || this.getDefaultSettings();
    s.llm.provider =
      document.getElementById("llm-provider")?.value || s.llm.provider;
    ...
    s.llm.maxTokens = parseInt(
      document.getElementById("max-tokens")?.value || s.llm.maxTokens,
      10
    );
    s.llm.temperature = parseFloat(
      document.getElementById("temperature")?.value || s.llm.temperature
    );
  ...
  async fetchGoogleModels() {
    // Return curated list of best Google Gemini models (no API call needed)
    // These are the most stable and recommended models as of 2024
    return [
  ...
  async fetchAIpipeModels() {
    ...
    const candidateSets = [];
    try {
      const r1 = await fetch("https://aipipe.org/openrouter/v1/models", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (r1.ok) {
        const d1 = await r1.json();
        // openrouter style: maybe list models objects
        if (Array.isArray(d1.models))
          candidateSets.push(d1.models.map((m) => m.name || m.id));
        else if (Array.isArray(d1))
          candidateSets.push(d1.map((m) => m.name || m.id));
      }
    } catch (e) {}

    try {
      const r2 = await fetch("https://aipipe.org/openai/v1/models", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (r2.ok) {
        const d2 = await r2.json();
        if (Array.isArray(d2.data))
          candidateSets.push(d2.data.map((m) => m.id || m.name));
      }
    } catch (e) {}
    ...
    const combined = Array.from(
      new Set([].concat(...candidateSets.filter(Boolean), fallback))
    );
    return combined;
  }
                ...
                <div class="conversation-item ${
                  conv.id === this.state.currentConversationId ? "active" : ""
                    ...
                    <div class="conversation-header">
                        <div class="conversation-title">${this.escapeHtml(
                          conv.title || "Conversation"
                        )}</div>
                        <button class="delete-conversation-btn" data-conversation-id="${
                          conv.id
      ...
      this.elements.conversationList
        ...
        .forEach((btn) => {
          btn.addEventListener("click", (e) => {
            ...
            this.showDeleteConfirmation(btn.dataset.conversationId);
          });
        });

      document.getElementById("total-conversations") &&
        (document.getElementById("total-conversations").textContent =
          this.state.conversations.size);
      const totalMessages = Array.from(
        this.state.conversations.values()
  ...
  updateChatHeader() {
    try {
      const conv = this.state.conversations.get(
        this.state.currentConversationId
      );
      if (conv) {
        document.getElementById("chat-title") &&
          (document.getElementById("chat-title").textContent = this.escapeHtml(
            conv.title || "Conversation"
          ));
  ...
  showDeleteConfirmation(conversationId) {
    try {
      ...
      // Update confirmation text with conversation title
      confirmText.innerHTML = `
                Are you sure you want to permanently delete the conversation:<br>
      ...
      // Add event listeners (remove previous ones to avoid duplicates)
      ...
      console.error("showDeleteConfirmation error", e);
    }
  }
  ...
  hideDeleteConfirmation() {
    try {
      ...
      console.error("hideDeleteConfirmation error", e);
    }
  }
  ...
  async confirmDeleteConversation(conversationId) {
    try {
      ...
      // Store conversation title for toast message
      ...
      if (this.state.currentConversationId === conversationId) {
        const remainingConversations = Array.from(
          this.state.conversations.values()
        );
      ...
      console.log(`🗑️ Conversation deleted: ${conversationTitle}`);
    } catch (e) {
      console.error("confirmDeleteConversation error", e);
  ...
  saveConversationHistory() {
    try {
      const conversations = Array.from(this.state.conversations.entries());
      localStorage.setItem(
        "agentflow_conversations",
  ...
  updateTheme() {
    // Always use dark theme for cyberpunk mode
  ...
  startVoiceInput() {
    try {
      ...
      this.speechRecognition.lang =
        this.state.settings.voice.language || "en-US";
      this.speechRecognition.start();
      document
        .querySelectorAll("#voice-toggle, #voice-input")
  ...
  showWelcomeMessage() {
    ...
    if (conv && conv.messages.length === 0) {
      this.addMessage(
        "assistant",
        "Welcome to CyberAgent! How can I assist you?"
      );
    }
  }
  ...
  showToast(type, title, message) {
    try {
      ...
      toast.innerHTML = `<div class="toast-icon"><i class="fas ${icon}"></i></div><div class="toast-content"><div class="toast-title">${this.escapeHtml(
        title
      )}</div><div class="toast-message">${this.escapeHtml(
        message
      )}</div></div><button class="toast-close" aria-label="Close">&times;</button>`;
  ...
  async handleFiles(files) {
    for (const file of files) {
      if (
        this.supportedFileTypes.some((type) =>
        ...
        try {
          ...
          this.addMessage(
            "system",
            `📄 Processing file: ${file.name} (${this.formatFileSize(
              file.size
            )})...`,
            convId
          );
          ...
          const fileMessage = `**📁 File: ${
            file.name
          }**\n\n\`\`\`${this.getFileLanguage(file.name)}\n${content}\n\`\`\``;

          this.addMessage("user", fileMessage, convId);

          // Add helpful prompt suggestion
          this.addMessage(
            "system",
            `✅ File "${file.name}" has been loaded. You can now ask questions about this file, request analysis, modifications, or explanations.`,
            convId
          );
  ...
  async readFileContent(file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        // Limit file size for context (max 100KB of text)
  ...
  updatePerformanceDisplay() {
    try {
      document.getElementById("response-time") &&
        (document.getElementById(
          "response-time"
        ).textContent = `${this.state.performance.responseTime}ms`);
      document.getElementById("memory-usage") &&
        (document.getElementById(
          "memory-usage"
        ).textContent = `${this.state.performance.memoryUsage}MB`);
      document.getElementById("api-calls") &&
        (document.getElementById("api-calls").textContent =
          this.state.performance.apiCalls || 0);
    } catch (e) {}
  }

  updateCharCount() {
    try {
      const count = this.elements.userInput
        ? this.elements.userInput.value.length
        : 0;
      ...
      case "edit":
        ...
        if (typeof msg.content === "string") {
          ...
          this.showToast(
            "info",
            "Edit",
            "Message loaded into input for editing."
          );
        } else {
          this.showToast(
            "warning",
            "Edit Not Supported",
            "Cannot edit structured/tool messages."
          );
        }
        break;
...
document.addEventListener("DOMContentLoaded", () => {
  window.agentFlow = new CyberAgent();
});
